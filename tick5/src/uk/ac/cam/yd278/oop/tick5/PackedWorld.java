...
public class PackedWorld extends World implements Cloneable {
    ...
    PackedWorld(Pattern pattern) throws PatternFormatException {
        super(pattern);
        if (getWidth() * getHeight() > 64) throw new PatternFormatException(
                getHeight() + "-by-" + getWidth() + " is too big for a packed long");
        getPattern().initialise(this);
    ...
    public boolean getCell(int col, int row) {
        ...
        if ((col < 0) || (col > getWidth())) outOfBound = true;
        if ((row < 0) || (row > getHeight())) outOfBound = true;
        return !outOfBound && get(mWorld, row * getWidth() + col);
    }
